...
description: "当用户分享了本地文件，即存在 `<|user_local_files` 时使用，本 skill 包括了本地文件系统读写规范和常见工具。"
---
...
Using `browserAgent.shared.userLocalFiles[key]` to get the file object (Handle or Entry). 

The user_local_files list in "- `${key}`: ${object.constructor.name}" format.

userLocalFiles 的 value 只可能存在这 4 种类型：
- FileSystemDirectoryHandle
- FileSystemFileHandle
- FileSystemDirectoryEntry
- FileSystemFileEntry

请根据对应的 Object 类型做相应的解析和操作

对于文件夹，不要一上来就递归读取
- 请先只查看一两层目录，看看情况
- 递归读取的时候，记得排除 `.git`, `node_modules` 等包含大量任务无关内容的目录
...
- Handle 接口才支持请求 `readwrite` 权限。Entry 接口不提供写权限，需要把改好的文件生成 Blob，触发下载，并让用户去手动覆盖
  - 如果更改文件比较多，可以按相对路径打包一个仅包含更改文件的 zip 下载给用户，让用户解压并手动合并
- 写入文件前必须确认目标文件和修改意图，并写一个简易的弹窗告知用户信息，让用户点击，以请求 `readwrite` 权限。
- 写权限同一页面内通常可复用；写入前仍应 queryPermission/requestPermission 检查
...
如果，有额外的 git 功能需求，你可以参考 `browser-git.js` 的代码自己实现

Safari 可能不支持查看隐藏的 `.git` 目录。
- 用户需要 git 相关需求的话，告知用户原因并推荐用户切换桌面版 Chrome。
